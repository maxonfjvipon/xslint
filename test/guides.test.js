...
/*
 * The size and the shape of the guides themselves, and so the one gate whose
 * subject is this repository's own documentation (#821). The root guide held
 * 198,202 characters before it, most of them a per-module archive loaded on
 * every turn, including every turn that never opens the module; and the gate
 * that read that file then — holding its counts of `ATTRIBUTES` and
 * `PATTERNS` to the lists in the code — said nothing at all about its size,
 * in the file that says every convention must be machine-enforced. What
 * answers it is a relocation and never a summary, those derivations being
 * what stops a later session loosening a bar that was placed on evidence:
 * each note moved into the `CLAUDE.md` of the directory its module sits in,
 * where it arrives when that directory is opened, and the root keeps the
 * rules, the bars, and one line per file. `LOADED` is the 150,000 characters
 * the harness warns past, and what arrives against it is a **chain** and not
 * a pair: the root guide, and the guide of every directory on the way down to
 * a file the turn touches, each injected once. That was measured rather than
 * assumed — two throwaway guides planted at `src/resources/` and
 * `src/resources/motives/`, neither of them ever read as a file, both arrived
 * the moment a motive under them was opened. The first spelling of the bar
 * weighed the root against the dearest single guide instead, which is a whole
 * directory short: it read 130,933 and called that 0.87 of the bar while a
 * turn touching `src/linters/` was loading 157,504 and over it. So the two
 * dearest notes moved one step further down, out of `src/CLAUDE.md` and into
 ...
 * reads 138,936, which is 0.93, and is `src/linters/`'s once more. What
 ...
 * since #832, so prose that has outgrown a guide does not simply move into
 ...
 * stands over it — 24,492 for `src/linters/CLAUDE.md`, where half of the bar
 * is 75,000 — and holds the root itself to 84,415, a number derived from the
 ...
 * fourth phase added 3,421 characters to `src/CLAUDE.md` and left the chain
 * reading 140,286 where two documents still said 136,418, the allowances
 ...
 * reads", so a gate asking whether *some* file still matches was satisfied by
 * the untouched one. Rewording only the first, to say 136,418 and 0.91 again,
 * left the figure wrong by 3,868 and the ratio by 0.03 with the suite reading
 * 16 passing: the drift gate had stopped reading that sentence and the anchor
 ...
 * and `src/syntax.js`, and the first to move because the bar said so rather
 * than in the change that wrote it. It said so a merge too late, though,
 * which is the half #844 answers: a gate that fires on the breach fires where
 * the relief no longer fits, the tree having to be red for as long as a
 * relocation takes to land. `ROOM` moves it off the breach, 10,000 characters
 * of headroom, so a chain reddens at 140,000 while there is still room to
 * answer it. It moves the bar rather than standing a second one beside it: a
 * breach gate left at 150,000 could not fire while this one is green, which
 * is what took `CEILING` out, so the two are one gate whose message names
 * both numbers. It is measured off the tree's own growth rather than picked:
 * over the 45 merges between #823 giving the chain its room back and this
 * one, the most a day of ordinary work added to the dearest chain was 5,298,
 * and 10,000 is 1.89 of that — inside the half-again-to-twice band every bar
 * here answers to, and held to it from both sides by `GROWN`, since a margin
 * under half again of a day is one a day crosses without warning and one past
 * twice it reddens a tree with room to spare. A day is the unit because that
 * is what a relocation takes to land, this tree merging three to eight times
 * in one. Twelve notes moved to make the bar quiet — `xpath-linter.js`,
 * `root-template-linter.js`, `corpus-linter.js` and `output-linter.js` out of
 * `src/linters/CLAUDE.md`, and `scaling.test.js`, `predicates.test.js`,
 ...
 * 1,064 characters of headroom, off a chain that is `src/linters/`'s own once
 ...
 * it — 2,147 characters out of every chain but the three standing under
 * `src/`, of which #877 has spent 1,910 on what it derives there. So the
 * relief lasted one ticket, and the merge behind the wildcard phase breached
 * it again by 162 with neither branch having crossed it alone either — the
 * same arithmetic a second time, which is what a shared budget does to two
 * branches that each measured themselves alone. What answered it was the move
 ...
 * of this file behind it, 5,286 characters out of every chain through `test/`
 ...
 * `src/linters/`'s chain 34 past a bar the dearest had stood 107 short of. A
 * row nobody would call prose is enough, which is the whole of what a shared
 * budget means. The move again answered it, on the dearest note that guide
 * held rather than on the newest — `output-linter.js`'s derivation into the
 * file-header note of `output-linter.js`, 3,276 characters out of that chain
 * and none out of the record. That is what the chain carries less of against
 ...
 * answers to the index and to its own directory both, so a derivation the
 * root has stopped pointing at turns red, and so does one standing where the
 * reader who needs it will never load it. `test/guides.js` is the walk itself
 * rather than a list of the five, since a guide left off a hand-written list
 * would take its claims out of every gate at once, and the reading of a claim
 * stands beside the walk: `worded` flattening a wrapped paragraph into one
 * line, `DOCUMENTS` naming the guides, the README and the release notes
 * together, `NEARBY` bounding how far past a name a number may stand and
 * still be a claim about it. Both gates ask it, so a count relocated into a
 * guide is judged where it went, and so is the length a guide states of the
 * file the line cap is lifted off, which is `test/conformance.test.js`'s to
 * ask beside the cap it is an exemption from. The counts came the other way
 * at that same sitting: `ATTRIBUTES` and `PATTERNS` are held to the lists in
 * the code from here now, the file that held them having reached the 1000
 * lines it caps every other file at (#825).
 *
 * The fourth breach is this branch's own, and the third in a row where
 * neither author crossed the bar alone: two commits of prose, each written
 * under it, merged with master to put the `test/` chain 2,779 past it and
 * `src/linters/`'s 927. Two moves answered it, one per chain and each the
 * dearest note its own guide could move — `src/xslint.js`'s derivation into
