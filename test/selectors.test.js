...
const SPLIT = [
  ...
  {
    xpath: '//(xsl:if|xsl:when)[xslint:normalize-space(@test) = "x"]',
    locals: ['if', 'when'],
    tail: '[xslint:normalize-space(@test) = "x"]',
  },
  {
...
const CANDIDATES = [
  ...
  'xslint:normalize-space(xsl:text) = "alpha"',
  ...
  'not(text()[xslint:normalize-space(.)]) and (@select or @as)',
