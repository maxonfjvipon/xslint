...
`test/budget.test.js`; the dearest reads 139,256, which is 0.93, and is `src/linters/`'s once
...
above weighs each other guide against the bar less what stands over it — 23,801 for
...
What that leaves is 744 characters of headroom, off a chain that is `src/linters/`'s own once
...
fires it: the nightly tier's derivation stands at the top of `test/budget.test.js` since, 2,238
