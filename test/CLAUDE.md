...
`test/budget.test.js`; the dearest reads 139,456, which is 0.93, and is `src/linters/`'s once
...
above weighs each other guide against the bar less what stands over it — 23,601 for
...
What that leaves is 544 characters of headroom, off a chain that is `src/linters/`'s own once
