...
**Stylesheet fixtures in tests** — every test stylesheet lives in a committed resource, never inline in the test source. Well-formed fixtures are `.xsl` files under `test/resources/` (linted by path, validated by the xcop workflow); malformed fixtures are committed YAML sources (a `content:` block scalar under `test/resources/malformed/`) read into a temp file, since a committed malformed `.xsl` cannot pass the `**/*.xsl` xcop workflow. The `fixtures` CI job fails the build when any `.test.js` file contains an inline `<?xml` or `<xsl:`. A committed `.xsl` is counted by the `should test default directory` test, so adding one bumps its `Processed files: N`.
