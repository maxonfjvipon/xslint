...
| `test/workflows.test.js` | Every job granted the scope its own steps write with, and left the scope they read with; every version the README pins of its own release reached by a rewrite of the `up` job; every substitution the release stamp spends held to reaching one place, and every release-notes command to naming a title |
